...
/// `TetherKit.app/Contents/MacOS/tetherkit-helper` (the plist's
...
enum OwnBundle {
    /// `…/TetherKit.app/Contents`, or nil when not running from an app bundle
...
/// /Library/PrivilegedHelperTools) that older TetherKit builds installed
...
/// Manages the `/usr/local/bin/tetherkit-cli` symlink.
///
...
///     TetherKit.app bundle or dangles. A regular file or a foreign symlink
///     (e.g. Homebrew's own tetherkit-cli) is reported, never clobbered.
...
enum CommandLineToolLink {
    ...
    /// Ours (points at a TetherKit app's bundled CLI) or dangling.
